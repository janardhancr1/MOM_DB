...
AS
	...
	FROM	#MOM_FRND_TEMP T 
		INNER JOIN MOM_FRND MF (NOLOCK)
		ON	T.FRND_MOM_USR_ID = MF.FRND_MOM_USR_ID
		INNER JOIN MOM_USR MGU (NOLOCK)
		ON	MF.MOM_USR_ID = MGU.ID
		AND	MGU.TYPE = 'G'
		INNER JOIN MOM_GRP MG (NOLOCK)
		ON	MGU.ID = MG.GRP_MOM_USR_ID
		INNER JOIN MOM_USR MU (NOLOCK)
		ON	MU.ID = T.FRND_MOM_USR_ID
	WHERE	MGU.BLOCKED	= 0
	AND	MG.BLOCKED	= 0
...
AS
	...
	SELECT	FRND_MOM_USR_ID = MF.MOM_USR_ID
	FROM	MOM_FRND MF (NOLOCK)
		LEFT JOIN MOM_USR MU (NOLOCK)
		ON	MF.MOM_USR_ID = MU.ID
		AND	MU.TYPE = 'G'
	WHERE	FRND_MOM_USR_ID = @MOM_USR_ID
	AND	MF.BLOCKED = 0
	...
	AND	MF.FRIDGE_OPTION & 1 = 1
	AND	MF.GRP_MOM_USR_ID IS NULL
	ORDER BY MF.ID DESC
	...
	SELECT	  TOP 4 
		...
		, MU.FULL_NAME
		, PICTURE = ISNULL (MU.PICTURE, '../images/q_silhouette.gif')
	INTO	#MOM_FRIENDS_TEMP
	FROM	#MOM_FRND_TEMP T
		INNER JOIN MOM_FRND MUF (NOLOCK)
		ON	T.FRND_MOM_USR_ID = MUF.MOM_USR_ID
		INNER JOIN MOM_USR MU (NOLOCK)
		ON	MUF.FRND_MOM_USR_ID = MU.ID
	...
	IF @@ROWCOUNT > 0
		SELECT	*
		FROM	#MOM_FRIENDS_TEMP
	ELSE
		SELECT	  TOP 4 
			  MU.ID
			, MU.FULL_NAME
			, PICTURE = ISNULL (MU.PICTURE, '../images/q_silhouette.gif')
		FROM	MOM_FRND MUF (NOLOCK)
			INNER JOIN MOM_USR MU (NOLOCK)
			ON	MUF.FRND_MOM_USR_ID = MU.ID
		WHERE	MUF.FRND_MOM_USR_ID <> @MOM_USR_ID
		AND	MUF.MOM_USR_ID <> @MOM_USR_ID
		AND	MU.TYPE <> 'G'
		ORDER BY NEWID()
	...
	SELECT	  TOP 4
		...
		, MU.DISPLAY_NAME
		, MFR.SHARE
		, GRP_MOM_USR_ID	= MGU.ID
		, MOM_GRP_NAME	= MGU.FIRST_NAME
	FROM	MOM_USR MU (NOLOCK)
		INNER JOIN MOM_FRND MF (NOLOCK)
		ON	MU.ID = MF.FRND_MOM_USR_ID
		INNER JOIN MOM_USR MGU (NOLOCK)
		ON	MF.MOM_USR_ID = MGU.ID
		AND	MGU.TYPE = 'G'
		INNER JOIN MOM_FRG MFR (NOLOCK)
		ON	MU.ID = MFR.MOM_USR_ID
		AND	MGU.ID = MFR.GRP_MOM_USR_ID
	WHERE	MFR.TYPE IN ('G', 'S')
	AND	MU.ID = @MOM_USR_ID
...
IF OBJECT_ID('SP_MOM_RCP_GET_RECENT') IS NOT NULL
	DROP PROC SP_MOM_RCP_GET_RECENT
GO

CREATE PROC SP_MOM_RCP_GET_RECENT
AS	
	SET TRAN ISOLATION LEVEL READ UNCOMMITTED
	SET NOCOUNT ON
...
IF OBJECT_ID('SP_MOM_RCP_GET_TOPRATED') IS NOT NULL
	DROP PROC SP_MOM_RCP_GET_TOPRATED
GO

CREATE PROC SP_MOM_RCP_GET_TOPRATED
AS	
	SET TRAN ISOLATION LEVEL READ UNCOMMITTED
	SET NOCOUNT ON
...
IF OBJECT_ID('SP_MOM_RCP_RATING') IS NOT NULL
	DROP PROC SP_MOM_RCP_RATING
GO

CREATE PROCEDURE SP_MOM_RCP_RATING
	  @MOM_RCP_ID	INT
	, @RATINGS		FLOAT
AS
	SET TRAN ISOLATION LEVEL READ UNCOMMITTED
...
IF OBJECT_ID('SP_MOM_TAGS_GET_BY_NAMESPACE') IS NOT NULL
	DROP PROC SP_MOM_TAGS_GET_BY_NAMESPACE
GO

CREATE PROCEDURE SP_MOM_TAGS_GET_BY_NAMESPACE
...
IF OBJECT_ID('SP_MOM_FRG_GET_BY_GRP_MOM_USR_ID') IS NOT NULL
	DROP PROC SP_MOM_FRG_GET_BY_GRP_MOM_USR_ID
GO

CREATE PROC SP_MOM_FRG_GET_BY_GRP_MOM_USR_ID
(
	  @MOM_USR_ID	BIGINT
	, @TYPE	CHAR (1)	= NULL
)
...
--	SELECT	@MOM_USR_ID
	...
	SELECT	*
	FROM	MOM_GRP (NOLOCK)
	WHERE	GRP_MOM_USR_ID = @MOM_USR_ID
	...
	WHERE	HIDE = 0
	--AND	(MF.TYPE = @TYPE OR @TYPE IS NULL)
	AND	(MF.FRIDGE_OPTION & 2 = 2)
	AND	MF.GRP_MOM_USR_ID = @MOM_USR_ID
	ORDER BY MF.ID DESC
...
BEGIN
	...
	SELECT	  A.ID
		...
		, U.FULL_NAME
		, TIME		= DBO.FN_MOM_DATE_DIFF(A.TIME, GETDATE())
		, ALBM_PHOTOS	= ISNULL((SELECT TOP 1 PHOTO_PATH FROM MOM_ALBM_PHTO T WHERE T.MOM_ALBM_ID = A.ID), '../images/nopic.jpg')
...
CREATE PROC SP_MOM_ALBM_PHTO_ADD
(
	    @MOM_ALBM_ID	INT
...
-------------------------------------------------------------------------------------------------------------------------------------------
IF OBJECT_ID('SP_MOM_ALBM_ADD') IS NOT NULL
	DROP PROC SP_MOM_ALBM_ADD
GO

CREATE PROC SP_MOM_ALBM_ADD
(
	    @MOM_USR_ID	BIGINT
	  , @TITLE	NVARCHAR (255)
	  , @DESCRIPTION	NVARCHAR (500)
...
-------------------------------------------------------------------------------------------------------------------------------------------
IF OBJECT_ID('SP_MOM_VID_ADD') IS NOT NULL
	DROP PROC SP_MOM_VID_ADD
GO

CREATE PROC SP_MOM_VID_ADD
(
	    @MOM_USR_ID	BIGINT
	  , @TITLE	NVARCHAR (255)
	  , @DESCRIPTION	NVARCHAR (500)
